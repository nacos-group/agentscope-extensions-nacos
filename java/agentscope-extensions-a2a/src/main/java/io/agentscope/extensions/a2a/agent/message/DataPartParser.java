...
public class DataPartParser implements PartParser<DataPart> {
    ...
    private ContentBlock parseToToolBlock(DataPart part) {
        ...
        String blockType = part.getMetadata().get(MessageConvertUtil.BLOCK_TYPE_METADATA_KEY).toString();
    ...
    private ContentBlock parseToToolResultBlock(DataPart part) {
        ...
        if (output instanceof String) {
            // Adapter Python Agentscope ToolResultBlock define, python tool result output spec is `str | List[TextBlock | ImageBlock | AudioBlock | VideoBlock]`
            builder.output(TextBlock.builder().text(output.toString()).build());
        } else if (output instanceof List) {
            @SuppressWarnings("unchecked") List<ContentBlock> outputList = (List<ContentBlock>) output;
